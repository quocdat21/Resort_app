...
const EditCategory: React.FC<EditCategoryProps> = ({ isOpen, onClose, onSuccess, category }) => {
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState<string | null>(null);
    const [iconFile, setIconFile] = useState<File | null>(null);
    const [iconPreview, setIconPreview] = useState<string | null>(category.iconUrl);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const response = await apiService.put(`/categories/${category.id}`, data);
            if (response.success) {
                onSuccess();
                onClose();
            } else {
            ...
            <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md">
                ...
                <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-xl font-bold text-slate-900">Chỉnh sửa loại phòng</h2>
                            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Cập nhật thông tin danh mục #{category.id}</p>
                        </div>
                    </div>
                    <button
                        onClick={onClose}
