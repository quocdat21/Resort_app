...
const EditRoom: React.FC<EditRoomProps> = ({ isOpen, onClose, onSuccess, room }) => {
    ...
    const [formData, setFormData] = useState({
        ...
        capacityAdults: room.capacity_adults.toString(),
        capacityChildren: room.capacity_children.toString(),
    ...
    const [selectedAmenities, setSelectedAmenities] = useState<number[]>(
        room.amenities ? room.amenities.map(a => a.id) : []
    );
                ...
                <div className="bg-white rounded-[32px] shadow-2xl w-full max-w-4xl h-[90vh] flex flex-col overflow-hidden animate-in fade-in zoom-in duration-300">
                    ...
                    <div className="flex items-center justify-between px-8 py-6 border-b border-slate-100 bg-slate-50/50 shrink-0">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h2 className="text-xl font-bold text-slate-900">Chỉnh sửa thông tin phòng</h2>
                                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Cập nhật thông tin template và hình ảnh cho toàn bộ các phòng con</p>
                            </div>
                        </div>
                        <button
                            onClick={onClose}
