...
const EditVoucher: React.FC<EditVoucherProps> = ({ isOpen, onClose, voucher, onSuccess }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await apiService.put(`/vouchers/${voucher.id}`, formData);

            if (response.success) {
                Swal.fire({
                    title: 'Cập nhật thành công!',
                    text: 'Thông tin voucher đã được lưu lại.',
                    icon: 'success',
                ...
                Swal.fire('Lỗi!', response.message || 'Không thể cập nhật voucher', 'error');
            }
        } catch (error: any) {
            console.error('Update voucher error:', error);
            Swal.fire({
                title: 'Lỗi!',
                text: error.response?.data?.message || 'Không thể cập nhật voucher. Vui lòng thử lại.',
                icon: 'error',
                confirmButtonColor: '#15803d'
            });
        } finally {
                ...
                >
                    ...
                    <div className="flex items-center justify-between px-8 py-6 border-b border-slate-100 bg-slate-50/50 shrink-0">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h2 className="text-xl font-bold text-slate-900 tracking-tight">Chỉnh sửa Voucher</h2>
                                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Cập nhật thông tin mã giảm giá</p>
                            </div>
                        </div>
                        <button
                            type="button"
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar p-8 space-y-8">
                        ...
                        <section className="space-y-6">
                            <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2">
                                <Ticket size={14} /> Thông tin cơ bản
                            </h3>
